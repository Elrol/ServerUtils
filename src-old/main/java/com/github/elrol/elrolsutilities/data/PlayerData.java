...
public class PlayerData implements IPlayerData {
    ...
    public String getTimePlayed() {
        return TextUtils.ticksToTime(minPlayed);
    }
    ...
    public boolean charge(double cost){
        ...
        setBal(balance - cost);
        Main.getLogger().info("Charged " + this.getDisplayName() + " " + TextUtils.parseCurrency(cost, false));
        return true;
    }

    public void pay(double amount){
        Main.getLogger().info("Paying " + this.getDisplayName() + " " + TextUtils.parseCurrency(amount, false));
        setBal(balance + amount);
        Main.getLogger().info("Paid " + this.getDisplayName() + " " + TextUtils.parseCurrency(amount, false));
        ...
        if(ModList.get().isLoaded(ModInfo.Constants.ftbchunkId)) {
            ...
            if(profile != null) {
                if (maxClaims != tempMaxClaims)
                    Main.mcServer.getCommands().performCommand(Main.mcServer.createCommandSource(), "ftbchunks admin extra_claim_chunks " + profile.getName() + " set " + tempMaxClaims);
                if (maxLoadedClaims != tempMaxLoadedClaims)
                    Main.mcServer.getCommands().performCommand(Main.mcServer.createCommandSource(), "ftbchunks admin extra_force_load_chunks " + profile.getName() + " set " + tempMaxLoadedClaims);
            }
        }
    ...
    public void tickJail() {
        ...
        if(isJailed()) {
            ...
            double dist = player.position().distanceTo(new Vector3d(loc.x(), loc.y(), loc.z()));
            if(max > 0 && dist >= max) {
    ...
    public ServerPlayerEntity getLastMsg() {
