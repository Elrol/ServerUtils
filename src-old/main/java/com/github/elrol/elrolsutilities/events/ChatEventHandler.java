...
public class ChatEventHandler {
    ...
    public void onPlayerChat(ServerChatEvent event) {
        ...
        if(data.usingStaffChat()) {
            TextUtils.sendToStaff(player.createCommandSource(), event.getMessage());
            event.setCanceled(true);
        } else {
            ITextComponent text = TextUtils.formatChat(player, event.getMessage());
            if (IElrolAPI.getInstance().getPermissionHandler().hasPermission(player.createCommandSource(), FeatureConfig.link_chat_perm.get()))
