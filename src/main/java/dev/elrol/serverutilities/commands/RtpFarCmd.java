...
public class RtpFarCmd
extends _CmdBase {
    public RtpFarCmd(ForgeConfigSpec.IntValue delay, ForgeConfigSpec.IntValue cooldown, ForgeConfigSpec.ConfigValue<List<? extends String>> aliases, ForgeConfigSpec.IntValue cost) {
    ...
    protected int execute(CommandContext<CommandSourceStack> c) {
        ...
        try {
            ...
            Main.textUtils.msg(c, Errs.not_player());
            return 0;
        }
        ...
        if (!data.hasBeenWarned()) {
            Main.textUtils.err(player, Errs.rtp_warning());
    ...
    implements Runnable {
        ...
        @Override
        public void run() {
            Methods.randomTeleport(this.player, CommandConfig.rtp_max.get(), CommandConfig.rtpfar_max.get());
        }
    }
