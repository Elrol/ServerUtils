...
import com.github.elrol.elrolsutilities.config.VoteConfig;
...
public class Votifier implements VoteHandler, VotifierPlugin {
    ...
    public void onVoteReceived(Vote vote, VotifierSession.ProtocolVersion protocolVersion, String remoteAddress) throws Exception {
        ...
        TextUtils.sendToChat(text);
        ServerPlayerEntity player = Methods.getPlayerFromName(vote.getUsername());
        if(player == null) {
    ...
    public void grantVoteReward(ServerPlayerEntity player) {
        Random rand = new Random();
        float chance = getTotalWeight() * rand.nextFloat();
