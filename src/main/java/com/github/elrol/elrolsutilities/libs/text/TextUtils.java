...
public class TextUtils {
    ...
    public static void staffChat(String message, UUID sender) {
        ...
        Main.serverData.staffList.forEach(uuid -> {
            ...
            staff.sendMessage(new TextComponent(text), sender);
        });
    }
    ...
    public static void sendToStaff(CommandSourceStack source, String message) {
        UUID uuid;
        try {
    ...
    public static TextComponent commandHelp(String[] content) {
        TextComponent text = new TextComponent(formatString(FeatureConfig.command_help_spacer.get() + "\n"));
    ...
    public static TextComponent formatChat(UUID uuid, String msg){
        IPlayerData data = Main.database.get(uuid);
    ...
    public static void msgNoTag(ServerPlayer player, BaseComponent message) {
        player.sendMessage(message, player.getUUID());
    }

    public static void action(ServerPlayer player, BaseComponent message) {
        player.sendMessage(message, ChatType.GAME_INFO, player.getUUID());
    }

    public static void msg(CommandContext<CommandSourceStack> context, BaseComponent translation) {
        TextUtils.msg(context.getSource(), translation);
    }

    public static void msg(ServerPlayer player, BaseComponent translation) {
        TextUtils.msg(player.createCommandSourceStack(), translation);
    }

    public static void msg(CommandSourceStack source, String text) {
        source.sendSuccess(new TextComponent(ModInfo.getTag() + text), false);
    }

    public static void msgNoTag(CommandSourceStack source, TextComponent text){
        source.sendSuccess(text, false);
    }

    public static void msg(CommandSourceStack source, BaseComponent translation) {
        source.sendSuccess(new TextComponent(ModInfo.getTag()).append(translation), false);
    }

    public static void err(CommandContext<CommandSourceStack> context, BaseComponent translation) {
        TextUtils.err(context.getSource(), translation);
    }

    public static void err(ServerPlayer player, BaseComponent translation) {
        TextUtils.err(player.createCommandSourceStack(), translation);
    }

    public static void err(CommandSourceStack source, String text) {
        source.sendFailure(new TextComponent(ModInfo.getTag() + ChatFormatting.RED + text));
    }

    public static void err(CommandSourceStack source, BaseComponent translation) {
        source.sendFailure(new TextComponent(ModInfo.getTag() + ChatFormatting.RED).append(translation));
    }

    public static void tab_msg(CommandSourceStack source, String string) {
        String tab = "    ";
        source.sendFailure(new TextComponent(tab + string));
    }

    public static void msg(CommandSourceStack source, String[] stringArray) {
        for (int i = 0; i < stringArray.length; ++i) {
            if (i == 0) {
                source.sendFailure(new TextComponent(ModInfo.getTag() + stringArray[0]));
                continue;
            }
    ...
    public static void sendMessage(CommandSourceStack source, ServerPlayer player, String msg) {
    ...
    public static void sendConfirmation(ServerPlayer player, Component[] lines) {
        UUID uuid = player.getUUID();
        TextComponent spacer = new TextComponent(ChatFormatting.AQUA + String.join("", Collections.nCopies(45, "#")));
        player.sendMessage(spacer, uuid);
        for(Component line : lines) player.sendMessage(line, uuid);
        player.sendMessage(spacer, uuid);
    }
